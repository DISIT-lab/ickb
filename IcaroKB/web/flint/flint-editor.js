...
function FlintEditor(container, imagesPath, config) {
	...
	function FlintSidebar(editor, config) {
		...
		this.updateKeywords = function(possibles, buttonCallback) {
			...
			allKeywords = [];
			if (mode === "sparql11query" || mode === "SPARQL 1.1 Query") {
		...
		this.updateProperties = function(datasetItem) {
			try {
				...
				if (datasetItem.properties == null) {
					...
					this.showActiveTab();
					var paramsData = {};
					paramsData[config.defaultEndpointParameters.queryParameters.query] = "SELECT DISTINCT ?p WHERE {?p a <http://www.w3.org/1999/02/22-rdf-syntax-ns#Property>} ORDER BY ?p";
					$
							.ajax({
								url : datasetItem.uri,
								data : paramsData,
                  ...
                  "Authorization" : "Basic a2ItcmVhZDppY2Fybw=="
								},
								dataType : 'json',
		...
		this.updateClasses = function(datasetItem) {
			try {
				...
				if (datasetItem.classes == null) {
					...
					this.showActiveTab();
					var paramsData = {};
					paramsData[config.defaultEndpointParameters.queryParameters.query] = "SELECT DISTINCT ?o WHERE {?o a <http://www.w3.org/2002/07/owl#Class> FILTER(isURI(?o))} ORDER BY ?o";
					$
							.ajax({
								url : datasetItem.uri,
								data : paramsData,
	...
	function Flint(container, imagesPath, config) {
		...
		try {
			...
			cm = new FlintCodeEditor(flint, "sparql11query");
			cm.display(editorId);
